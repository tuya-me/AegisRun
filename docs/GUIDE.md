...
.lang-btn { display: inline-block; padding: 6px 20px; background: #f0f0f0; border-radius: 20px; text-decoration: none; color: #333; font-size: 14px; margin: 0 4px; cursor: pointer; }
...
> **Two levels in one:** [Beginner Level](#beginner-level) — 3 min onboarding + troubleshooting
> [Detailed Level](#detailed-level) — library API, CLI, WASI sandbox, web panel
...
Press `1` to see interception in action, `3` for 7 security scenarios, `0` to exit.

### Quick Troubleshooting

1. **Check MoonBit installation** — `moon version` should show `0.1.20260608` or higher.
   Reinstall: `irm https://cli.moonbitlang.com/install/powershell.ps1 | iex`

2. **Check working directory** — `cd D:\moonbit\aegisrun` then `dir quickstart.cmd`

3. **Clear build cache** — `rmdir /s /q target`
...
| Output doesn't match code | Stale build cache | `rmdir /s /q target` |
| `Package "lib" not found` | File not under `src/` | Only `src/` packages use `@lib` |
| `abstract type` error | Missing `pub` on struct fields | Add `pub` for cross-package access |
| Two files both have `main` | One entry per package | Rename the extra one |
...
| `aegisrun.exe demo` | Policy engine demo (14 checks) |
| `aegisrun.exe scan malware.py` | Scan script for threats |
| `aegisrun.exe serve` | Web admin panel |
| `aegisrun.exe policy show` | View current policy |
| `moon run src/demo/intercept.mbt` | 12 attack interception demo |

### Environment Variables

| Variable | Purpose | Example |
|----------|---------|---------|
| `AEGISRUN_POLICY` | Sync policy from web panel | `set AEGISRUN_POLICY=domain_bl=evil.com\|path_bl=/etc/passwd` |
| `AEGISRUN_DEMO` | Select single demo (1-7) | `set AEGISRUN_DEMO=2` |
| `AEGISRUN_ATTACKS` | Select attack types (1-12) | `set AEGISRUN_ATTACKS=1,4,7,12` |
...
**AI Agent Developer** — check third-party tools before execution:
```moonbit
let domain_ok = @lib.check_domain(bl, suffixes, "evil.com")  // → Deny
```

**Security Admin** — scan suspicious scripts:
```cmd
aegisrun.exe scan suspicious.py
```

**Policy Manager** — open web dashboard at `http://localhost:9090`:
```cmd
aegisrun.exe serve
```
...
```
src/lib/*.mbt          ← MoonBit library (policy engine, for `moon add`)
runtime/src/lib.rs     ← Rust library (policy engine, for `cargo add`)
runtime/src/main.rs    ← CLI tool (aegisrun.exe)
dashboard.html         ← Web admin panel
```

**The library is the product; CLI and web panel are remote controls.**

```
AI Agent (Claude/GPT)
    │  MCP Protocol
    ▼
AegisRun Policy Engine
    │  check_domain / check_path / check_env
    ▼
Allow → Execute  /  Deny → Blocked
```

### Using as a MoonBit Library

**Install:** `moon add aegisrun`
...
|----------|---------|:-------:|
| `check_domain(bl, suffixes, domain)` | Domain in blacklist? | Allow/Deny |
| `check_path(bl, prefixes, path)` | Path in blacklist? | Allow/Deny |
| `check_env(sensitive, varname)` | Env var sensitive? | Allow/Deny |
| `check_tool_id(bl, id)` | Tool banned? | Allow/Deny |
| `new_policy_engine()` | Create policy engine | PolicyState |
| `load_policy_patch(state, str)` | Load exported string | void |

### Using as a Rust Library

**Install:** `cargo add aegisrun-runtime`
...
| Function | Purpose | Returns |
|----------|---------|---------|
| `Policy::standard()` | Create standard preset | Policy |
| `Policy::strict()` | Create strict preset | Policy |
| `Policy::permissive()` | Create permissive preset | Policy |
| `.check_domain("evil.com")` | Domain check | bool |
| `.check_path("/etc/passwd")` | Path check | bool |
| `.check_env("OPENAI_API_KEY")` | Env var check | bool |
| `scan_script(&source)` | Scan for threats | Vec\<ScanFinding\> |
| `sandbox_read_file(&policy, path)` | Safe file read | Result\<String\> |

### CLI Tools

**Build:** `cd runtime && cargo build`

**All Commands:**
```cmd
aegisrun.exe demo                  # Policy engine demo
...
aegisrun.exe policy show           # View policy
aegisrun.exe policy set strict     # Switch preset
aegisrun.exe policy block domain evil.com  # Block domain
```

### WASI Sandbox (True Physical Isolation)
...
The tool cannot access any files or environment variables — WASI zero preopens + filtered env. This is not a policy Deny, it's physical isolation at the WASI layer.

### Web Admin Panel

```cmd
aegisrun.exe serve    # Then http://localhost:9090
```

Features: defense toggles, real-time stats, domain/path/env policy management, audit log viewer. Edit in browser → click "Copy CLI commands" → paste in terminal → instant sync.
...
> **Note:** MCP runs on the Rust side. MoonBit compiles to Wasm and has no TCP capability.
> Start with: `cd runtime && cargo run -- serve`

### Policy Customization

| Method | Path | Best for |
|--------|------|----------|
| Edit YAML preset | `presets/standard.yaml` | Project init |
| CLI | `moon run src/main deny domain xxx` | Quick temporary bans |
| Web panel | `cargo run -- serve` → browser | Non-developers |
...
```
D:\moonbit\aegisrun\
├── src/lib/               MoonBit library (9 modules)
├── src/demo/              Interception demo
├── src/generator/         Malicious script generator
├── runtime/               Rust library + CLI
│   ├── src/lib.rs         Rust library
│   ├── src/main.rs        CLI tool
│   ├── src/server.rs      Web panel + MCP
│   ├── src/sandbox.rs     wasmtime sandbox
│   └── src/persist.rs     Audit + persistence
├── dashboard.html         Web admin panel
├── quickstart.cmd         One-click menu
├── README.md              Project overview
└── docs/                  Full documentation
    ├── GUIDE.md           This file
    ├── ARCHITECTURE.md    Architecture + call chain
...
> **双级合一：** [新手入门](#新手入门) — 3 分钟上手 + 排障
> [详情参考](#详情参考) — 库 API、CLI、WASI 沙箱、网页面板
...
1. **确认 MoonBit 安装正确** — `moon version` 应显示 `0.1.20260608` 或更高。
   重装：`irm https://cli.moonbitlang.com/install/powershell.ps1 | iex`
